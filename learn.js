// ฟังก์ชันสำหรับควบคุมการสลับแท็บความรู้
function openTab(evt, tabName) {
    // 1. ซ่อนเนื้อหาของแท็บทั้งหมดที่มีอยู่ก่อนหน้า
    const tabContents = document.getElementsByClassName("tab-content");
    for (let i = 0; i < tabContents.length; i++) {
        tabContents[i].classList.remove("active");
    }

    // 2. เอาคลาส 'active' ออกจากปุ่มกดทั้งหมด
    const tabButtons = document.getElementsByClassName("tab-btn");
    for (let i = 0; i < tabButtons.length; i++) {
        tabButtons[i].classList.remove("active");
    }

    // 3. แสดงเนื้อหาของแท็บที่ผู้ใช้เลือกในปัจจุบัน
    document.getElementById(tabName).classList.add("active");

    // 4. ใส่คลาส 'active' ให้กับปุ่มที่ถูกกดเพื่อแสดงสถานะไฮไลท์
    evt.currentTarget.classList.add("active");
    
    // 5. เลื่อนหน้าจอกลับขึ้นไปด้านบนเบาๆ เพื่อความลื่นไหลในการอ่านเนื้อหาใหม่
    window.scrollTo({
